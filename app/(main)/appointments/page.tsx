...
const DUMMY_DATA = Array.from({ length: 23 }).map((_, index) => ({
  _id: `dummy-${index + 1}`,
  title: [
    "Modern Luxury Villa", "Comfortable Apartment", "Spacious Family Home", 
    "Urban Studio", "Premium Land Plot", "Heritage Colony House"
  ][index % 6],
...
//   const allProperties = useMemo(() => [...realProperties, ...DUMMY_DATA], [realProperties]);
...
//   return (
//     <div className="max-w-7xl mx-auto px-4 py-8 space-y-6 min-h-screen font-sans antialiased">
      
//       {/* FILTER BAR */}
//       <div className="flex flex-col md:flex-row gap-3 items-center bg-card p-3 rounded-2xl border shadow-sm">
//         <div className="relative flex-1 w-full">
//           <Search className="absolute left-3 top-1/2 -translate-y-1/2 text-muted-foreground" size={16} />
//           <Input 
//             placeholder="Search properties..." 
//             className="pl-10 h-11 border-none bg-muted/40 focus-visible:ring-1 rounded-xl text-sm"
//             value={searchQuery}
//             onChange={(e) => { setSearchQuery(e.target.value); setCurrentPage(1); }}
//           />
//         </div>
...
//         <div className="flex gap-2 w-full md:w-auto">
//           <Select onValueChange={(v) => { setLocationFilter(v); setCurrentPage(1); }}>
//             <SelectTrigger className="w-full md:w-48 h-11 rounded-xl text-xs font-bold border-muted">
...
//           <Button
//             variant="outline"
//             className="h-10 px-5 rounded-xl font-bold border-muted"
//             disabled={currentPage === 1}
//             onClick={() => { setCurrentPage(prev => prev - 1); window.scrollTo(0,0); }}
//           >
...
//           <div className="flex items-center gap-2 px-4 text-xs font-black bg-muted/50 h-10 rounded-xl border">
//             <span className="text-primary">{currentPage}</span>
//             <span className="text-muted-foreground">/</span>
//             <span>{totalPages}</span>
//           </div>
